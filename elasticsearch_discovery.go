...
func main() {
	...
	klog.Info("Kubernetes Elasticsearch cluster discovery")
	...
	serviceName := os.Getenv("ELASTICSEARCH_MASTER_SERVICE_NAME")
	if serviceName == "" {
	...
	// If we did not find an elasticsearch service then log a warning
	// and return without adding any unicast hosts.
	if elasticsearch == nil {
		klog.Warningf("Failed to find the %s service: %v", serviceName, err)
		return
	}
